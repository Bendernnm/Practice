...
function FunctionsSystemApproach(arrayValue, functionValue, stackFunction, low, up) {
    this.low = low;
    this.up = up;
...
FunctionsSystemApproach.prototype = {
    calculateValueF: function () {
        arrayY = [];
        for (var i = 0; i < this.arrayX.length; i++) {
            arrayY[i] = this.f(this.arrayX[i]);
    ...
    sumFunctionsOnAllPoints: function (indexFirst, indexSecond) {
        var f1 = this.stackFunction[indexFirst];
        var f2 = this.stackFunction[indexSecond];
        var f = function (value) {
            return f1(value) * f2(value);
        };
        return calculateIntegral(this.low, this.up, 100, f);
    },
    sumFunctionAndFOnAllPoints: function (index) {
        var f1 = this.stackFunction[index];
        var f2 = this.f;
        var f = function (value) {
            return f1(value) * f2(value);
        };
    ...
    showTable: function () {
        var tableColumn = 4;
        var div = document.getElementById("divShow3");
        div.innerHTML = "";
